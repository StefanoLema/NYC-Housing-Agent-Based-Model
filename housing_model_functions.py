...
def model_conversions(population, income):
    ...
    Returns:
    tuple: A tuple containing two converted DataFrames: Population DataFrame with proportions instead of actual numbers. 
           Income DataFrame with the first column as average income values and second column as population proportion.
    """
    
    # Create copies to prevent inplace modifications
...
def initial_price_dist(agents_df, year, price_data, type = 'median'):
    '''
    Assign an initial house price to agents based on their district placement.
    ...
    # Create copy to prevent inplace modifications
...
def initial_affluence_dist(n, a, p, agents_df):
    ...
    agents_df (DataFrame): DataFrame with columns 'x_coord', 'y_coord'
...
def update_house_prices(agents_df, weight, radius, amenities_df, amenity_weights, amenity_radius):
    ...
    radius (float): Radius within which to calculate neighborhood average (in transformed km).
    amenities_df (DataFrame): DataFrame with columns ['latitude', 'longitude', 'type'].
    amenity_weights (dict): Dictionary mapping amenity types to their weights.
    amenity_radius (float): Radius within which amenities influence house prices (in transformed km).
...
def random_positions(df, point_idx):
    ...
    available_indices = np.delete(all_indices, point_idx)
...
def perform_swaps(agent_df, point_idx, random_indices):
    ...
    Returns:
    Tuple: df (DataFrame): Updated DataFrame after performing the swap (if applicable), Boolean term for labling if a swap was made.
    '''
...
def new_iteration(agent_df, weight, radius, amenities_df, amenity_weights, amenity_radius):
    '''
    Perform swaps on the affluence grid and calculate the new house values, including amenity influence.
    
    Inputs:
    agent_df (DataFrame): DataFrame with columns ['latitude', 'longitude', 'affluence', 'price']
...
def grid_initializer(num_agents, num_amenities, pop_dist, affluence_vals, affluence_dist, residence_df, amenities_df, mult = False):
    ...
    distribution and affluence values. The agents are placed in residential areas, and 
    each agent is assigned an initial affluence distribution. The amenities are placed in amenity areas
    according to type. The function also calculates the appropriate radii for neighborhood  and amenity calculations 
    based on the maximum distance between the furthermost agents.

    Inputs:
    num_agents (int): The total number of agents to be placed in the grid.
    
    num_amenities (int): The total number of amenities to be placed in the grid.

    pop_dist (pd.DataFrame): A series containing the population distribution across districts.

    affluence_vals (np.array): An array containing different affluence values.

    affluence_dist (np.array): An array representing the initial affluence distribution for agents.

    residence_df (pd.DataFrame): A dataframe containing the residential areas data, including district and residential area geometries.

    amenities_df (pd.DataFrame): A dataframe containing amenity data, with columns for geometry, Type, and District.
    
    mult: (int) Used to test if initial affluence distribution matters. Returns mult number of initial affluence grids. By default False.
    
    Returns:
    agents_df (GeoDataFrame): A GeoDataFrame containing the coordinates and district of each placed agent, with an added 'affluence' column.
    
    amenities_df (DataFrame): A DataFrame containing the coordinates and type of each placed amenity.
    
    radius (float): The calculated radius, used for calculating neighborhood value.
    
    radius_amenity (float): The calculated amenity radius, used for calculating ammenity contribution.
    
    """
    # Make copies to prevent inplace changes
    ...
    # Assumption that amenity radius is smaller than neighborhood radius. For 1000 agent model this translates to a 
    # roughly 5 km agent radius and 1 km amenity radius which works very well with our model.
...
def available_residences(agent_df, district_list, residences, growth_rate):
    ...
    agent_df (pd.dataframe): dataframe with model agents, columns latitude, longitude, District, affluence, price
    
    district_list (list): ordered list of districts
    
    residences (pd.dataframe): dataframe with residence locations.
    
    growth_rate (pd.dataframe): dataframe with percentage growth rate for each district
    ...
    # Calculate centroids for all agent_df
    ...
    for district in district_list:
        ...
        # Extract growth rate for borough k and calculate annualized rate
...
def add_agents(agents_df, radius, agents_needed, residences, a, p, year, price_data, type='median'):
    ...
    a (np.array): Array of affluence levels.
    p (np.array): Probabilities corresponding to the affluence levels.
...
def calculate_ape(array1, array2):
    ...
    Parameters:
    array1 (np.array): First array of numerical values.
    array2 (np.array): Second array of numerical values (of the same length as array1).
